    ...
    if (images) {
      images.forEach(function(img) {
        gms.push(function(cb) {
          ...
          var thumbPath = 'thumb' + path.sep + img;
          var basePath = path.dirname(thumbPath).split("/");
          var verifiedPath = "";
          for(var ix = 0; ix < basePath.length; ix++) {
            var pathToVerify = path.join(opts.out, verifiedPath, basePath[ix]);
            fs.mkdirSync(pathToVerify);
            verifiedPath = path.join(verifiedPath, basePath[ix]);
          }
