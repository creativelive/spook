...
// bump the default of 10
process.setMaxListeners(50);

function casperCmds(argv, cb) {
  var cmds = {};
  console.log(argv.tests);
  async.parallel({
    includes: function(cb) {
      // remove quoted args present to prevent shell variable expansion
    ...
    tests: function(cb) {
      // remove quoted args present to prevent shell variable expansion
      argv.tests = argv.tests.replace(/\'/g, '');
      glob(argv.tests || '*', {
    ...
    var args = [];
    if (res.includes) {
      args.push('--includes=' + res.includes.join(','));
    }
    args = args.concat(argv._);

    if (argv['list-tests'] && !argv.record) {
      console.log('SPOOK will run:');
    ...
    res.tests.forEach(function(test) {
      ...
      cmds[name] = {
        // slimerjs chokes if relative paths to tests do not start './'
...
function Spook(opts, cb) {
  ...
  opts.base = opts.base || '';
  // remove any trailing slash from end of path
          ...
          .on('data', function(ln) {
            ...
            if (match) {
              ...
              if (match[1].substr(0, 1) === '/') {
                ...
                // strip preceeding './' or opts.base length
                testFile = match[1].substr(3 + (opts.base.length || -1));
