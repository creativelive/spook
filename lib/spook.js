...
function casperCmds(argv, cb) {
  ...
  async.parallel({
    ...
    tests: function(cb){
      if(argv.tests) {
        cb(null, argv.tests.split(','));
      } else {
        var cwd = path.join(argv.cwd, argv.base || '');
        glob('*', {
          cwd: cwd
        }, cb);
      }
...
function Spook(opts, cb) {
  ...
  function save(cb) {
    ...
    cb(null, run);
    // find a way to handle the sigint event - drop run.json somewhere sync'ly?
    // have server ingest on startup?
  }
  var tests;
  ...
  spook.run = function runner(cb) {
    ...
    tests.forEach(function(test, index){
      ...
      fns.push(function(cb){
        ...
        if(opts.record){
          ws[test] = fs.createWriteStream(path.join(opts.out, pad(3, index, '0') + '.' + test + '.log'), {flags: 'a'});
    ...
    if(opts.work === 'series') {
      ...
      async.parallelLimit(fns, 3, function(err, res){
        save(function(err, res){
          cb(err, run);
        });
      });
